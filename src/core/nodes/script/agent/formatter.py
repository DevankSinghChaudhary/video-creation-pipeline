...
from core.nodes.script.state.Formatter import FormatterState
...
def Formatter(state: GlobalInformationState):
    ...
    print(f'[Formatter] Started Successfully')
    st = time.time()
    ...
    print(f'[Formatter] Finished Successfully')
    print(f'[Formatter] {time.time()-st}')
